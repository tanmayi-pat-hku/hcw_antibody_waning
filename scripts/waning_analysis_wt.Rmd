...
median_df <- data.frame(x = factor(1), median = median_svnt)

post_infection_svnt <- ggplot(valid_dates_result, aes(x = factor(1), y = rX_svnt_wt)) + 
  geom_jitter(width = 0.2, alpha = 0.8, size = 2.5, colour = "#006400") + 
  geom_crossbar( data = median_df, aes(x = x, y = median, ymin = median, ymax = median), width = 0.5, colour = "black", size = 0.5) +
  geom_point(aes(x = factor(1), y = median_svnt), colour = "black", size = 3) + 
  scale_x_discrete(labels = c("Post-Infection"), expand = expansion(add = c(0.5, 0.5))) + 
  scale_y_continuous(limits = c(0, 110), breaks = seq(0, 100, by = 20)) + labs(x = NULL, y = "WT sVNT Inhibition (%)") + 
  theme_bw(base_size = 12) + 
  theme( plot.caption = element_text(face = "bold", size = 14, hjust = 0.5), 
...
svnt_wt_combined_plot <- (svnt_baseline_wt + post_infection_svnt + one_svnt_wt_plot + 
                  ...
                  three_svnt_wt_plot + 
                  four_svnt_wt_plot) + 
  plot_layout(widths = c(1.0, 0.5, 1.0, 1.0, 2.0, 2.0), ncol = 6) + plot_annotation(title = "WT sVNT", 
...
ggsave("svnt_wt_combined_plot.pdf", plot = svnt_wt_combined_plot, width = 18 , height = 6)
```
...
elisa_wt_combined_plot <- (elisa_baseline_wt + one_elisa_wt_plot + 
                         two_elisa_wt_plot + 
                         three_elisa_wt_plot) + 
  plot_layout(ncol = 4) + 
...
ggsave("elisa_wt_combined_plot.pdf", plot = elisa_wt_combined_plot, width = 15, height = 6)
```
