...
## Community Post-Infection Analysis 2022 
...
#Plot post-infection ELISA Value for 2022
```{r}
comm_infection_elisa_plot <- ggplot(community_data_elisa, aes(x = factor(1), y = i1_elisa_wt)) + 
...
```{r}
comm_infection_svnt_plot <- ggplot(community_data_svnt, aes(x = factor(1), y = i1_svnt_wt)) + 
  geom_jitter(width = 0.2, alpha = 0.8, size = 2.5, colour = "darkred") + 
...
#POST INFECTION in Cohort 

#Source Earliest Date Function
```{r}
source("scripts/helper/fc_earliest_date.R")
```


#Calculate Earliest Date Data 
```{r}
first_infection_data <- calculate_earliest_date(data)
```

#Source Extract Date Function
```{r}
...
#sVNT cohort Post infection 1 results 
```{r}
...
#ELISA cohort Post infection 1 results
```{r}
