...
Analysis of both sVNT and ELISA results after one to four doses of the inactivated (S) and mRNA (B) vaccines were recorded for the study population. 
...
```{r}
# Test for differences in slopes (interaction term)
model_slope <- lm(weight ~ days_since_dose1 * dose3_brand * type, 
                  data = three_dose_waning_svnt)
anova(model_slope)

# Compare slopes between all group combinations
pairwise_slopes <- emtrends(model_slope, pairwise ~ dose3_brand * type, 
                           var = "days_since_dose1")
summary(pairwise_slopes, adjust = "tukey")


###############################################################

model_better <- lm(weight ~ days_since_dose1 * dose3_brand + 
                   days_since_dose1 * type + 
                   dose3_brand * type, 
                 data = three_dose_waning_svnt)

pairwise_slopes <- emtrends(model_better, pairwise ~ dose3_brand * type, 
                           var = "days_since_dose1")
summary(pairwise_slopes, adjust = "none")
...
```{r Supplementary Figure 4 Prep, echo = FALSE, warning = FALSE, message = FALSE, include = FALSE}
source("scripts/cleaned/figure_sup3.R")
```

```{r Supplementary Figure 4, echo = FALSE, warning = FALSE, message = FALSE}
figure_sup3
```
