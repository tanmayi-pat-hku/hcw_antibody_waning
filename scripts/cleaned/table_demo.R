...
demographic_table <- bind_rows(
  one_dose_demo %>% select(Dose, Group, n, male_pct, female_pct, under50_pct, over50_pct) %>% mutate(pct_of_total = one_dose_pct$pct_of_total),
  two_dose_demo %>% select(Dose, Group, n, male_pct, female_pct, under50_pct, over50_pct) %>% mutate(pct_of_total = two_dose_pct$pct_of_total),
  three_dose_demo %>% select(Dose, Group, n, male_pct, female_pct, under50_pct, over50_pct) %>% mutate(pct_of_total = three_dose_pct$pct_of_total),
  total_demo %>% mutate(Dose = "Total", Group = "", pct_of_total = 100) %>% select(Dose, Group, n, male_pct, female_pct, under50_pct, over50_pct, pct_of_total)
)
