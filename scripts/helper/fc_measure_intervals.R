  ...
  for (i in 1:11) {
    ...
    if (r_date_col %in% names(data) && weight_col %in% names(data)) {
      ...
      temp_result <- data %>%
        ...
        select(id, measure_date = .data[[r_date_col]], weight = .data[[weight_col]], days_since_dose1, permutation = .data[[permutation_col]], dose1_brand, dose2_brand, dose3_brand, dose4_brand, dose5_brand, dose6_brand, type)
